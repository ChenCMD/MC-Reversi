...
#CursorEntityがある = 置けるマスがある ときの処理
    #一番最後まで読んだ後最良値を返しつつ一つ戻って他の候補の先の盤面を掘る処理
        execute if entity @e[type=area_effect_cloud,tag=Candidate_201,tag=!InactiveCandidate_201,distance=..50,x=-19995.0,y=11.0,z=21.0] if score #RemainingDepth ReversiData_201 matches ..0 run schedule function reversi_201:game/turn/ai/advanced/deep.back 1t
    #まだ掘る処理
        #5箇所まで絞る
        ...
        #更に掘る
            execute if entity @e[type=area_effect_cloud,tag=Candidate_201,tag=!InactiveCandidate_201,distance=..50,x=-19995.0,y=11.0,z=21.0] if score #RemainingDepth ReversiData_201 matches 1.. run function reversi_201:game/turn/ai/advanced/deep.init
