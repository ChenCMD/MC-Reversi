...
#CursorEntityがある = 置けるマスがある ときの処理
    ...
    #まだ掘る処理
        ...
        #無効化
            execute if entity @e[tag=Candidate_201,tag=!InactiveCandidate_201] if score #RemainingDepth ReversiData_201 matches 1.. run tag @e[tag=TestPoint_201,tag=!InactiveTestPoint_201,limit=1] add InactiveTestPoint_201
        #更に掘る
