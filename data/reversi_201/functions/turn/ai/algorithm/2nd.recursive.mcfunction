#チェックシステム
    #そのマスが石を置けるかのチェック
        execute positioned ~ ~ ~ unless block ~ ~ ~ #reversi_201:stone_block run function reversi_201:turn/player/check.run
...
#つぎ メモ: ここどうしても複数tickに跨がせることが出来ないから処理数だけ減らして無理やりやる
    ...
    execute at @s unless block ~ ~ ~ orange_stained_glass run function reversi_201:turn/ai/algorithm/2nd.recursive