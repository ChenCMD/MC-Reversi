...
#Lv2+処理
    execute if score #RemainingDepth ReversiData_201 matches 1.. run scoreboard players operation #MoreDepthTurnColor ReversiData_201 = $TurnColor ReversiData_201
        ##DEBUG ホログラムkill
        #execute if score #RemainingDepth ReversiData_201 matches 1.. if score $StoneQuantity ReversiData_201 < #AI-LastAlgorithm ReversiData_201 as @e[tag=Candidate_201] unless score @s SortValue_201 matches 0..4 at @s positioned ~ ~35.5 ~ run kill @e[limit=1,tag=Display,distance=..0.25]
    ...
    execute if score #RemainingDepth ReversiData_201 matches 1.. run function reversi_201:turn/ai/advanced/deep.init