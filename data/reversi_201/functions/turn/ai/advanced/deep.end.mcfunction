...
#置けるマスがない時の処理 最悪値を持つダミー召喚してbackの実行
    ...
    execute if entity @e[tag=DummyCandidate_201] run function reversi_201:turn/ai/advanced/deep.back
#CursorEntityがある = 置けるマスがある ときの処理
    #一番最後まで読んだ後最良値を返しつつ一つ戻って他の候補の先の盤面を掘る処理
        execute if entity @e[tag=Candidate_201,tag=!InactiveCandidate_201] if score #RemainingDepth ReversiData_201 matches ..0 run schedule function reversi_201:turn/ai/advanced/deep.back 1t
    #まだ掘る処理
        #5箇所まで絞る
            execute if entity @e[tag=Candidate_201,tag=!InactiveCandidate_201] if score #RemainingDepth ReversiData_201 matches 1.. run function reversi_201:turn/common/sort.run
                ##DEBUG ホログラムkill
                #execute if entity @e[tag=Candidate_201,tag=!InactiveCandidate_201] if score #RemainingDepth ReversiData_201 matches 1.. if score $StoneQuantity ReversiData_201 < #AI-LastAlgorithm ReversiData_201 as @e[tag=Candidate_201] unless score @s SortValue_201 matches 0..4 at @s positioned ~ ~35.5 ~ run kill @e[limit=1,tag=Display,distance=..0.25]
        ...
        #更に掘る
            execute if entity @e[tag=Candidate_201,tag=!InactiveCandidate_201] if score #RemainingDepth ReversiData_201 matches 1.. run function reversi_201:turn/ai/advanced/deep.init
