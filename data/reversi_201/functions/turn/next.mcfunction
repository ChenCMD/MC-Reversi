#CursorEntityがある = 置けるマスがある ときの処理
    ...
    #カーソル見えるように
        execute if entity @e[tag=CursorEntity_201] as @e[tag=NoVisible_201] at @s run tp @s ~ ~2 ~
        #execute if entity @e[tag=CursorEntity_201] as @e[tag=SelectItem_201] run data modify entity @s Glowing set value 1b
#置けるマスがない時の処理
    #各色の石の数store
        execute unless entity @e[tag=CursorEntity_201] store result score $StoneBlack ReversiData_201 if entity @e[tag=Stone_Black_201]
        execute unless entity @e[tag=CursorEntity_201] store result score $StoneWhite ReversiData_201 if entity @e[tag=Stone_White_201]
    #どちらかの石が0の時の処理
        #メッセージとサウンド
    ...
    #0じゃない = たまたま置けない時の処理
        ...
        #次のターンにする
            ...
            execute unless entity @e[tag=CursorEntity_201] unless score $StoneBlack ReversiData_201 matches 0 unless score $StoneWhite ReversiData_201 matches 0 run function reversi_201:turn/check